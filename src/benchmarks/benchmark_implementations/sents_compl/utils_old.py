...
WORD2VEC_PATH = "extras/GoogleNews-vectors-negative300-hard-debiased.bin.gz"
...
from .metrics import (
    ...
    toxicity_fraction,
    get_sentiment_score,
    get_gender_polarity_score,
    eval_gender_polarity,
    eval_sentiment,
    get_sentiment,
    get_gender_polarity,
)
...
def get_agg_metrics(data, columns, metric):
    ...
    for key in columns:
        ...
        if metric == "gini":
            res["gini_" + key] = gini_coefficient(values)
        if metric == "avg":
